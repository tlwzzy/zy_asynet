...
class NCaltech101:
    ...
    def generate_input_representation(self, events, shape):
        ...
        elif self.event_representation == 'concat':
            return np.concatenate((self.generate_event_histogram(events, shape),self.generate_event_queue(events, shape)),axis=2)
        if self.event_representation == 'voxel_grid':
            return self.generate_voxel_grid(events, shape)
        if self.event_representation == 'voxel_grid_cat_gistogram':
            return np.concatenate((self.generate_voxel_grid(events, shape),self.generate_event_histogram(events, shape)),axis=2)
        if self.event_representation == 'hist_cat_voxel':
            return np.concatenate((self.generate_event_histogram(events, shape),self.generate_voxel_grid(events, shape)),axis=2)
    ...
    @staticmethod
    def generate_voxel_grid(events, shape, B=5, device='cuda', sensor_size=(180,240), temporal_bilinear=True):
        """
        Events: N x 4, where cols are x, y, t, polarity, and polarity is in {0,1}. x and y correspond to image
        coordinates u and v.
        """
